...
export function initPhoneChrome() {
    ...
    if (!wrap) {
        ...
        wrap.innerHTML = `
            <button type="button" id="pp-mail-chip" class="pp-mail-chip" title="Open Phone Trigger — drag to move" aria-label="Open mail">
    ...
    if (!document.getElementById('pp-mail-chip')) {
    ...
    root = wrap;
    wireMailChip(document.getElementById('pp-mail-chip'));
...
function wireMailChip(chip) {
    ...
    const onMove = (ev) => {
        ...
        if (!moved && (Math.abs(dx) > 5 || Math.abs(dy) > 5)) {
            moved = true;
            chip.classList.add('pp-mail-chip-dragging');
        }
        if (!moved) {
            return;
        }
...
function applyChipPosition(chip) {
    const s = getSettings();
    // null/undefined must NOT use Number() — Number(null) === 0 and pins the chip to the top.
    const x = s.mailChipX;
    const y = s.mailChipY;
    const hasCustom = x != null && y != null
        && Number.isFinite(Number(x))
        && Number.isFinite(Number(y));

    if (hasCustom) {
        chip.style.left = `${Number(x)}%`;
        chip.style.top = `${Number(y)}%`;
        chip.style.right = 'auto';
        chip.style.bottom = 'auto';
        chip.style.transform = 'none';
...
function wireForceMailPanel(view) {
    ...
    sendBtn?.addEventListener('click', () => {
        const contact = listMailContacts().find((c) => c.key === (fromSelect?.value || forceFromKey))
            || listMailContacts()[0];
        if (!contact) {
