...
import { isPhoneWaveContact, PHONEWAVE_NAME, playDmailSequence } from './dmail.js';
...
function renderForceMailPanelHtml() {
    ...
    const options = contacts.length
        ? contacts.map((c) => {
            ...
            return `<option value="${escapeHtml(c.key)}"${selected}>${escapeHtml(c.name)}</option>`;
        }).join('')
        : `<option value="">—</option>`;
    ...
    return `
        <div class="pp-force-panel" id="pp-force-panel">
            <label class="pp-field">
                ...
                <select id="pp-force-from">${options}</select>
            </label>
            <label class="pp-field">
...
function wireForceMailPanel(view) {
    ...
    const fromSelect = /** @type {HTMLSelectElement|null} */ (view.querySelector('#pp-force-from'));
    ...
    sendBtn?.addEventListener('click', () => {
        const contact = listMailContacts()
            .filter((c) => !isPhoneWaveContact(c.key))
            .find((c) => c.key === (fromSelect?.value || forceFromKey))
            || listMailContacts().find((c) => !isPhoneWaveContact(c.key));
        if (!contact) {
...
function renderCompose(view) {
    ...
    const active = contacts.find((c) => c.isActive && !isPhoneWaveContact(c.key))
    ...
    const options = contacts.length
        ? contacts.map((c) => {
            ...
            const label = isPhoneWaveContact(c.key) ? c.name : `${c.name}${mark}`;
            return `<option value="${escapeHtml(c.key)}"${selected}>${escapeHtml(label)}</option>`;
        }).join('')
    ...
    const syncDmailUi = () => {
        const dmail = isPhoneWaveContact(toSelect?.value);
        if (hint) {
            hint.hidden = !dmail;
        }
        if (sendBtn) {
    ...
    toSelect?.addEventListener('change', syncDmailUi);
    syncDmailUi();
    ...
    const send = async () => {
        sync();
        const key = toSelect?.value || '';
        const toName = toSelect?.selectedOptions?.[0]?.textContent?.replace(/\s*★\s*$/, '').trim() || '';
        const dmail = isPhoneWaveContact(key) || isPhoneWaveContact(toName);
        if (!composeBody.trim()) {
        ...
        try {
            ...
            if (dmail) {
                // Persist first, then fullscreen divergence (video + end distortion)
                await sendUserEmail({
                    to: toName,
                    toKey: key,
                    subject,
...
function renderMemory(view) {
    ...
    view.innerHTML = `
        ...
        <div class="pp-memory-toolbar">
            <label class="pp-field pp-memory-char">
                <span class="pp-label">Character</span>
                <select id="pp-memory-char">${options}</select>
            </label>
    ...
    view.querySelector('#pp-memory-char')?.addEventListener('change', (ev) => {
        memoryCharKey = /** @type {HTMLSelectElement} */ (ev.target).value || '';
        render();
    });
