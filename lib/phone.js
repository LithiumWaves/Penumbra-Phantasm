...
function isGenericDesign() {
    return getSettings().phoneDesign === 'generic';
}

function applyPhoneDesign() {
    const generic = isGenericDesign();
    const wrap = document.getElementById('pp-phone-root');
    const chassis = document.querySelector('.pp-chassis');
    const overlay = getOverlay();
    wrap?.classList.toggle('pp-design-generic', generic);
    chassis?.classList.toggle('pp-design-generic', generic);
    overlay?.classList.toggle('pp-design-generic', generic);
    wrap?.setAttribute('data-pp-design', generic ? 'generic' : 'keitai');
    chassis?.setAttribute('data-pp-design', generic ? 'generic' : 'keitai');
    if (generic) {
...
function softLeft() {
    ...
    } else if (screen === 'settings' || screen === 'wallpaper' || screen === 'sounds' || screen === 'dmail' || screen === 'handset') {
        screen = 'home';
        render();
...
function renderSettings(view) {
    ...
    const dmailMeta = showPw
        ...
        : 'Hidden';
    const designMeta = isGenericDesign() ? 'Generic' : 'Keitai';

    view.innerHTML = `
        <div class="pp-title">Settings</div>
        <div class="pp-scroll">
            <button type="button" class="pp-row" data-goto="handset">
                <span>Handset</span>
                <span class="pp-row-meta">${escapeHtml(designMeta)} →</span>
            </button>
...
function renderHandsetSettings(view) {
    setSoftLabels('Home', 'Back');
    view.style.background = '';
    view.className = 'pp-view';
    const design = isGenericDesign() ? 'generic' : 'keitai';

    view.innerHTML = `
        <div class="pp-title">Handset</div>
        <div class="pp-scroll">
            <p class="pp-settings-note">Shell look for the phone UI. Generic skips the flip animation.</p>
            <button type="button" class="pp-row ${design === 'keitai' ? 'pp-selected' : ''}" data-design="keitai">
                <span class="pp-toggle-copy">
                    <span class="pp-toggle-title">Keitai</span>
                    <span class="pp-toggle-blurb">Red flip-phone chassis.</span>
                </span>
                <span class="pp-row-meta">${design === 'keitai' ? '●' : ''}</span>
            </button>
            <button type="button" class="pp-row ${design === 'generic' ? 'pp-selected' : ''}" data-design="generic">
                <span class="pp-toggle-copy">
                    <span class="pp-toggle-title">Generic</span>
                    <span class="pp-toggle-blurb">Neutral slab phone for plain e-mail use.</span>
                </span>
                <span class="pp-row-meta">${design === 'generic' ? '●' : ''}</span>
            </button>
        </div>
    `;

    view.querySelectorAll('[data-design]').forEach((btn) => {
        btn.addEventListener('click', () => {
            const next = btn.getAttribute('data-design') === 'generic' ? 'generic' : 'keitai';
            const s = getSettings();
            s.phoneDesign = next;
            saveSettings();
            applyPhoneDesign();
            toast(next === 'generic' ? 'Generic handset' : 'Keitai handset', 'success');
            render();
        });
    });
}
