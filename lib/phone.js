...
export function initPhoneChrome() {
    ...
    // Migrate old overlay shells (div, or pre-red chassis without lattice).
    const needsRebuild = overlay && (
        String(overlay.tagName || '').toUpperCase() !== 'DIALOG'
        || !overlay.querySelector('.pp-lattice')
    );
    if (needsRebuild) {
    ...
    view.innerHTML = `
        ...
        <div class="pp-scroll pp-compose">
            ...
            <label class="pp-field">
                ...
                <textarea id="pp-compose-body" rows="7">${escapeHtml(composeBody)}</textarea>
            </label>
