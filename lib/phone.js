...
/** Scale the SG-001 chassis so the full phone fits in the visible viewport (incl. mobile safe areas). */
export function fitPhoneToViewport() {
    ...
    const availW = Math.max(160, (vv?.width ?? window.innerWidth) - pad * 2);
    const availH = Math.max(200, (vv?.height ?? window.innerHeight) - pad * 2);

    const w = chassis.offsetWidth;
    const h = chassis.offsetHeight + 16; // close button overhang
    if (!w || !h) {
        return;
    }
...
export function initPhoneChrome() {
    ...
    wrap.innerHTML = `
        ...
        <div id="pp-overlay" class="pp-overlay" hidden>
            <div class="pp-phone" role="dialog" aria-label="SG-001 phone">
