...
/** Pending character-card Apply confirmation */
    ...
    const send = async () => {
        ...
        try {
            ...
            if (dmail) {
                ...
                pendingCardPreview = result.acceptResult?.cardPreview || null;
                ...
                if (tiers.apply && pendingCardPreview) {
                    openPhone('dmail-card');
                    return;
                }
...
function renderDmailSettings(view) {
    ...
    const rows = [
        ...
        },
        {
            ...
            on: tiers.apply,
            title: 'Apply (hard)',
            blurb: 'Write facts into lorebook “Phone Trigger Worldline”, then ask before editing the character card. Never pastes into chat.',
        },
    ];
    ...
    view.innerHTML = `
        ...
        <div class="pp-scroll">
            <p class="pp-settings-note">
                ...
                <strong>Propose</strong> = review gate for the writers;
                <strong>Worldline</strong> = soft prompt inject;
                <strong>Apply</strong> = lorebook + confirmed character-card write.
                Soft and hard can both be on. Propose delays both until you confirm.
            </p>
            ${rows.map((r) => `
    ...
    view.querySelectorAll('[data-tier]').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            if (key === 'showPhoneWave') {
                s.showPhoneWave = s.showPhoneWave === false;
                saveSettings();
                if (s.showPhoneWave === false && isPhoneWaveContact(composeToKey)) {
                    composeToKey = '';
                }
                toast(s.showPhoneWave !== false ? 'PhoneWave shown' : 'PhoneWave hidden', 'info');
                render();
                return;
            }
            const tierKey = /** @type {'dmailSpectacle'|'dmailWorldline'|'dmailPropose'|'dmailApply'} */ (key);
            setDmailTier(tierKey, !Boolean(s[tierKey]));
            render();
        });
    });
...
function renderDmailPropose(view) {
    ...
    const finish = async (accept) => {
        if (accept) {
            ...
            pendingCardPreview = result.cardPreview || null;
            const bits = [];
            if (result.worldline) {
                bits.push('overlay');
            }
            ...
            proposeFact = '';
            proposeSubject = '';
            if (result.cardPreview && getDmailTiers().apply) {
...
/**
 * Separate confirmation before writing the worldline block onto the character card.
 * @param {HTMLElement} view
 */
function renderDmailCardConfirm(view) {
    ...
    view.style.background = '';
    view.className = 'pp-view';
    const preview = pendingCardPreview || previewWorldlineCharacterCard();
    ...
    const blockPreview = preview.block
        ? escapeHtml(preview.block).replace(/\n/g, '<br>')
        : '<em>(empty — would clear the worldline block)</em>';

    view.innerHTML = `
        <div class="pp-title">Character card</div>
        <div class="pp-scroll pp-compose">
            <p class="pp-settings-note">
                Lorebook is already updated. Confirm writing this worldline block onto
                <strong>${escapeHtml(preview.characterName)}</strong>'s description.
            </p>
            <div class="pp-card-preview">${blockPreview}</div>
    ...
    view.querySelector('#pp-card-confirm')?.addEventListener('click', async () => {
        const ok = await applyWorldlineToCharacterCard();
        pendingCardPreview = null;
