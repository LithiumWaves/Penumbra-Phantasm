...
function keypadHtml() {
    return KEYPAD.map((row) => `
        <div class="pp-key-row">
            ${row.map((k) => `<button type="button" class="pp-key" data-digit="${k}" aria-label="Key ${k}"><span>${k}</span></button>`).join('')}
        </div>
    `).join('');
}
...
/** Scale chassis inside the top-layer dialog so the full SG-001 fits. */
export function fitPhoneToViewport() {
    const phone = document.querySelector('.pp-phone');
    const chassis = document.querySelector('.pp-chassis');
    const overlay = getOverlay();
    if (!phone || !chassis || !isPhoneOpen()) {
        return;
    }

    phone.style.transform = 'none';
    phone.style.setProperty('--pp-scale', '1');
    ...
    phone.style.setProperty('--pp-scale', String(scale));
...
function presentOverlay(overlay) {
    ...
    applyOverlayViewportBox(overlay);
    // <dialog>.showModal() → browser top layer, escapes ST transform containing blocks
    // that left fixed UIs as a strip at the bottom on S25 Ultra (Killer-Within fix).
    try {
        ...
        console.warn('[Penumbra-Phantasm] dialog.showModal failed; using open attribute', err);
    }
    try {
...
function buildPhoneInnerHtml() {
    return `
        <div class="pp-phone" aria-label="SG-001 phone">
            <button type="button" id="pp-close" class="pp-close" title="Close phone">✕</button>
            <div class="pp-chassis" aria-label="Okabe Rintaro Model SG-001">
                <div class="pp-rail pp-rail-left" aria-hidden="true"></div>
                <div class="pp-rail pp-rail-right" aria-hidden="true"></div>
                <div class="pp-topbar">
                    <div class="pp-speaker" aria-hidden="true"></div>
                    <div class="pp-cam-block" aria-hidden="true">
                        <span class="pp-cam pp-cam-main"></span>
                        <span class="pp-cam pp-cam-sub"></span>
                        <span class="pp-flash"></span>
                    </div>
                </div>
                <div class="pp-model">SG-001</div>
                <div class="pp-screen">
                    <div class="pp-status">
                ...
                <div class="pp-mic" aria-hidden="true"></div>
            </div>
        </div>
    `;
}
...
function bindChromeControls(scope) {
    $(scope).find('#pp-close').off('click.pp').on('click.pp', () => closePhone());
...
export function initPhoneChrome() {
    ...
    // Migrate pre-dialog markup (div overlay) so users who already loaded v0.1.1 still recover.
    if (overlay && String(overlay.tagName || '').toUpperCase() !== 'DIALOG') {
        overlay.remove();
        overlay = null;
        overlayWired = false;
    }
    ...
    if (!overlay) {
        ...
        overlay.setAttribute('aria-label', 'SG-001 phone');
        overlay.innerHTML = buildPhoneInnerHtml();
        // Append to body (not inside transformed ST containers) — dialog top layer also helps.
...
function renderHome(view) {
    ...
    view.innerHTML = `
        <div class="pp-home-brand">PHONE TRIGGER</div>
        <div class="pp-home-sub">SG-001 · Ready</div>
