...
export function playDmailSequence() {
    return new Promise((resolve) => {
        ...
        video.addEventListener('error', () => {
            ...
            if (!triedFallbackSrc) {
                ...
                if (retry && typeof retry.catch === 'function') {
                    retry.catch(() => { /* wait for hang timeout / error */ });
                }
                return;
            }
        ...
        if (playPromise && typeof playPromise.catch === 'function') {
            // Autoplay rejection is not fatal — keep loading; hang timeout handles true failure
            playPromise.catch((err) => {
                console.debug('[Phone Trigger] D-Mail video play deferred', err?.name || err);
            });
        }

        // Give a large local mp4 time to buffer before falling back to CSS meter
        window.setTimeout(() => {
