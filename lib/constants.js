 ...
 * {{description}} {{personality}} {{scenario}} {{char_card}} {{chat_summary}} {{call_memory}}
 */
...
export const DEFAULT_SETTINGS = Object.freeze({
    ...
    /**
     * Cap D-Mail body at 36 printable ASCII (English) characters — Steins;Gate phone limit.
     * On by default.
     */
    dmailLimit36: true,
    /**
     * Where PhoneWave “delivers” the D-Mail cosmetically.
     * - self: appear in the user’s inbox (as a received D-Mail)
     * - otherwise a mail-contact key: character’s phone (no user inbox entry)
     */
    dmailRedirectKey: 'self',
    /**
     * When true (default), character-routed D-Mails are added to that character’s
     * mail memory list but left unchecked (manual mode excluding those ids).
     */
    dmailMemoryUnchecked: true,
    /**
     * When true, D-Mails also appear in the user’s outbox (debug).
     * Default off — D-Mails are transmissions, not normal sent mail.
     */
    dmailShowInOutbox: false,
     ...
     * so characters are aware of the current story. Fine-tune picks on phone
     * Inject context → Mail gen.
     */
    injectChatIntoMail: true,
    /** How many recent chat messages to include in the mail/call auto summary. */
    chatSummaryMessages: 12,
    /** Inject chat summary into live call prompts (separate from mail). */
    injectChatIntoCall: true,
    /** Inject selected / auto e-mails into live call prompts. */
    injectMailIntoCall: true,
    /** Max e-mails auto-fed into a call prompt. */
    callMailContextMax: 8,
    /** Inject call memory summaries into mail generation prompts. */
    injectCallMemoryIntoMail: true,
    /** Inject prior call memory into live call prompts. */
    injectCallMemoryIntoCall: true,
