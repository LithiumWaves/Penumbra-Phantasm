 ...
 * @returns {Promise<import('./store.js').EmailMessage & { isDmail?: boolean, worldlineFact?: string }>}
 */
export async function sendUserEmail({ to, toKey, subject, body, inReplyTo, requestReply = true }) {
    ...
    if (dmail) {
        const fact = await deriveWorldlineFact({
            subject: outbound.subject,
            body: outbound.body,
        });
        await recordDmail(outbound, fact);
        // Worldline/Apply deferred to phone flow when Propose is on;
        // otherwise accept immediately per tier toggles.
        const tiers = getDmailTiers();
        if (!tiers.propose && (tiers.worldline || tiers.apply)) {
            await acceptWorldlineFact(fact, tiers);
        }
        return Object.assign(outbound, { isDmail: true, worldlineFact: fact });
    }
