...
import { buildMailReplyPrompt, buildMailForcePrompt, generateMailText } from './generate.js';
 ...
 * Default scope: only e-mails to/from the character who is about to speak.
...
export function buildMailSystemPrompt(forContact = null) {
    ...
    if (scope === 'speaker') {
        ...
        // Group chat with no identifiable speaker → inject nothing (privacy).
        if (ctx.groupId && !contact) {
            return '';
        }
        // Solo chat fallback to active character
        if (!contact) {
    ...
    const filterForSpeaker = (list) => {
        if (scope !== 'speaker' || !contact) {
            return list;
        }
        return list.filter((e) => emailInvolvesContact(e, contact));
    };

    const recentIn = filterForSpeaker(getInbox()).slice(0, max);
    const recentOut = filterForSpeaker(getOutbox()).slice(0, max);
    const combined = [...recentIn, ...recentOut]
        .sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0))
        .slice(0, max * 2);
    const unread = filterForSpeaker(getInbox()).filter((e) => !e.read).length;
    ...
    const vars = {
        ...
        mail_count: String(recentIn.length + recentOut.length),
        send_instructions: sendInstructions,
        scope: scope === 'speaker' ? 'speaker' : 'all',
    };
