...
function emptyStore() {
    return { emails: [], memorySelections: {}, dmails: [], worldlineFacts: [], version: 3 };
}
...
/**
 * Contacts for the To: picker — PhoneWave (D-Mail) plus active-chat characters.
 * Group chat → group members. Solo chat → the active character only.
 * PhoneWave is always present (exact name: "PhoneWave (name subject to change)").
 */
export function listMailContacts() {
    ...
    // Permanent D-Mail channel — always first in the list
